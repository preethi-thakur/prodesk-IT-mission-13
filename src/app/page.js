...
const nav = [
  ...
  ["Team", UserGroupIcon, "/profile"],
];
const avatarColors = ["violet", "cyan", "orange", "pink"];
const memberNames = ["Jamie Lee", "Sam Lee", "Nora Patel", "Chris Kim"];
  ...
  return (
    <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
      <div className="brand">
        ...
        <b>TaskMatrix</b>
        <button className="mobile-close" onClick={() => dispatch(toggleSidebar())}><XMarkIcon /></button>
      </div>
            ...
            <div className="drawer-body">
              <form onSubmit={handleSave} style={{ display: "grid", gap: 16 }}>
                ...
                <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }}>
                  <label style={{ display: "grid", gap: 8, fontSize: 12, fontWeight: 700 }}>
                    Manager Name
                    <input value={form.managerName} onChange={(event) => updateField("managerName", event.target.value)} style={{ border: "1px solid #e6e8ec", borderRadius: 8, padding: "10px 12px", background: "#fff" }} required />
                  </label>
                  <label style={{ display: "grid", gap: 8, fontSize: 12, fontWeight: 700 }}>
                    Assignee
                    <input value={form.assignee} onChange={(event) => updateField("assignee", event.target.value)} style={{ border: "1px solid #e6e8ec", borderRadius: 8, padding: "10px 12px", background: "#fff" }} required />
                  </label>
                </div>
                <label style={{ display: "grid", gap: 8, fontSize: 12, fontWeight: 700 }}>
                  Due date
    ...
    <main className={dark ? "app dark min-h-screen" : "app min-h-screen"}>
      ...
      <div className="content">
        <header className="topbar">
          ...
          <button className="icon-button" onClick={() => dispatch(toggleTheme())}>{dark ? <MoonIcon /> : <MoonIcon />}</button>
        ...
        <section className="page-head">
          ...
          <div className="head-actions">
            ...
            <button className="primary" onClick={() => handleCreateTask("todo")}><PlusIcon />New task</button>
          </div>
        </section>
        ...
        <div className="tabbar">
          ...
          <span />
          <button className="view-button"><Squares2X2Icon />View</button>
        </div>
        {tab === "Board" ? <Board tasks={tasks} onToast={setToast} onCreateTask={handleCreateTask} /> : <section className="empty"><CalendarDaysIcon /><h2>{tab} view</h2><p>This project view is ready for your team’s work.</p></section>}
      </div>
